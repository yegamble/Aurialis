  ...
  return (
    <div className="space-y-4 p-1 overflow-y-auto max-h-[calc(100vh-200px)]">
      ...
      <Section title="Dynamics">
        ...
        <Slider label="Threshold" value={params.threshold ?? -18} min={-40} max={0} step={0.1} unit="dB" onChange={(v) => onParamChange("threshold", v)} />
        <Slider label="Ratio" value={params.ratio ?? 3} min={1} max={20} step={0.1} unit=":1" onChange={(v) => onParamChange("ratio", v)} />
        <Slider label="Attack" value={params.attack ?? 20} min={0.1} max={100} step={0.1} unit="ms" onChange={(v) => onParamChange("attack", v)} />
        <Slider label="Release" value={params.release ?? 250} min={10} max={1000} step={1} unit="ms" onChange={(v) => onParamChange("release", v)} />
        <Slider label="Makeup" value={params.makeup ?? 0} min={-12} max={12} step={0.1} unit="dB" onChange={(v) => onParamChange("makeup", v)} />
      </Section>
      ...
      <Section title="Stereo" defaultOpen={false}>
        <Slider label="Width" value={params.stereoWidth ?? 100} min={0} max={200} step={1} unit="%" onChange={(v) => onParamChange("stereoWidth", v)} />
        <Slider label="Bass Mono Freq" value={params.bassMonoFreq ?? 200} min={50} max={500} step={1} unit="Hz" onChange={(v) => onParamChange("bassMonoFreq", v)} />
        <Slider label="Mid Gain" value={params.midGain ?? 0} min={-12} max={12} step={0.1} unit="dB" onChange={(v) => onParamChange("midGain", v)} />
        <Slider label="Side Gain" value={params.sideGain ?? 0} min={-12} max={12} step={0.1} unit="dB" onChange={(v) => onParamChange("sideGain", v)} />
      </Section>

      {/* Output / Limiter */}
      <Section title="Output">
        <div className="flex gap-2 mb-3 flex-wrap">
          {["Spotify", "Apple Music", "YouTube", "SoundCloud", "CD"].map((p) => (
            <button
              key={p}
        ...
        <Slider label="Target LUFS" value={params.targetLufs ?? -14} min={-24} max={-6} step={0.1} unit="LUFS" onChange={(v) => onParamChange("targetLufs", v)} />
        <Slider label="Ceiling" value={params.ceiling ?? -1} min={-6} max={0} step={0.1} unit="dBTP" onChange={(v) => onParamChange("ceiling", v)} />
        <Slider label="Release" value={params.limiterRelease ?? 100} min={10} max={500} step={1} unit="ms" onChange={(v) => onParamChange("limiterRelease", v)} />
      </Section>
    </div>
  );
}
