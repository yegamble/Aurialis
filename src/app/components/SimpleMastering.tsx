...
export function SimpleMastering({
  ...
  const handleKnobInteraction = (e: React.MouseEvent | React.TouchEvent) => {
    ...
    const diff = center - clientY;
    const newVal = Math.max(0, Math.min(100, intensity + diff * 0.5));
    onIntensityChange(Math.round(newVal));
  };

  // Knob rotation: 0 = -135deg, 100 = 135deg
  const knobRotation = -135 + (intensity / 100) * 270;
